...
export const IdeaMarketRegister: React.FC<IdeaMarketRegisterProps> = () => {
  ...
  const [formData, setFormData] = useState<IdeaMarketDetail | null>(null);
  ...
  const {
    ...
    enabled: !!ideaId,
    onSuccess: (post) => setFormData(post),
  });

  const mutation = useMutation({
    mutationFn: () => putPostIdeaMarket(Number(ideaId), formData!),
    onSuccess: () => {
  ...
  if (isError || !formData) return <div>게시글을 불러올 수 없습니다.</div>;
